...
import { Check, TrendingUp, Users, ArrowLeft, Zap } from "lucide-react";
...
const plans = [
  ...
  },
  {
    ...
    period: "/mois",
    icon: Users,
    description: "Pour ouvrir l'accès collaboratif à votre espace Astryd avec un coach entrepreneurial certifié.",
    features: [
      "Tout de Cap",
    ...
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-background">
        ...
        <div className="flex-1 flex flex-col">
          <DashboardHeader currentPage="Formules" />
            ...
            <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
              <div className="text-center space-y-3 md:space-y-4 px-4">
                ...
                <p className="text-muted-foreground text-sm sm:text-base md:text-lg max-w-2xl mx-auto">
                  Astryd vous accompagne dans votre cheminement entrepreneurial. Choisissez la formule qui correspond à vos besoins.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 px-4">
                {plans.map((plan) => {
                  ...
                  return (
                    <Card 
                      key={plan.name} 
                      className={`p-4 sm:p-6 relative ${plan.current ? 'border-primary' : ''} ${plan.badge === 'Populaire' ? 'border-accent' : ''} flex flex-col`}
                    >
                      {plan.badge && (
                        <Badge 
                          variant={plan.badge === 'Gratuit' ? 'secondary' : plan.badge === 'Populaire' ? 'default' : 'outline'}
                          className="absolute -top-3 right-4 sm:right-6 text-xs"
                        >
                          {plan.badge}
                        </Badge>
                      )}
              ...
              <Card className="p-4 sm:p-6 bg-muted/30 mx-4">
                <div className="text-center space-y-2 sm:space-y-3">
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    Contactez-nous à <a href="mailto:contact@astryd.ai" className="text-primary hover:underline break-all">contact@astryd.ai</a>
                  </p>
                </div>
              </Card>
            </div>
